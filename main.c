...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
    ...
    // e) wait for switch to be released
		...
		ClearReady();
		WaitForASHigh();
		Delay1ms(205);       // f) wait 250ms
		SetVT();             // g) VT signal goes high
		Delay1ms(205);       // h) wait 250ms
...
void WaitForASLow(void){
// write this function
		while (GPIO_PORTF_DATA_R == 0x00000019) {
	}
}
...
void SetVT(void){
// write this function to set Ventricular trigger
...
// Notes:   friendly means it does not affect other bits in the port
void ClearVT(void){
// write this function to clear Ventricular trigger
	GPIO_PORTF_DATA_R &= ~0x00000013;
